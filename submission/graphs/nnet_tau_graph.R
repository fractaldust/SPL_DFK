...
calc.loss = function(known, tau_c, pars){
    # known : known dataset, training set
    # tau_c : tau category, numeric 1-6
    # pars  : pair of optimal parameters (from tuning)
    v   = tau_c
    k   = 3         
    i   = 1      
    tr  = known
    ...
    res  = list("loss.list"   = loss, 
                "parameters"  = data.table("size"  = pars[1],
                                           "decay" = pars[2]))
    return(res)
}
...
hight = 1 
b = 11
ggsave(file="./graphs/plots/find_tau.pdf",  plot,
