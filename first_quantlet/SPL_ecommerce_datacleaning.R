...
user = joint[, list(user_nr.obs = .N, user_dob = user_dob), by = "user_id"]
...
user$user_age_c = split(user$user_age_c, user$user_age, c, tag)
...
user.k = joint.k[, list(k_nr.obs = .N, user_retrate = mean(return)), by = "user_id"]
c = c(0, 0.17, 0.38, 0.58, 0.79, 1.1)
tag = c("very low", "low", "normal", "high", "very high")
user.k$user_retrate_c = split(user.k$user_retrate_c, user.k$user_retrate, c, tag)
...
user$user_retrate_c = factor(user$user_retrate_c, levels = c(tag, "unknown", "new"))

# variables that are going to be used in prediction
user.final = user[, list(user_id = user_id, user_retrate = as.factor(user_retrate_c), user_age = as.factor(user_age_c), 
...
# item_avg.price
item = joint[, list(item_nr.obs = .N, item_avg.price = mean(item_price), item_size = item_size), by = "item_id"]

# item type depending on size system
# shoes    : 36:42, 5,5+,...,9,
# clothes  : s,m,l,xl,xxl, S, M,...,XXL
# unsized  : "unsized", "other"
shoes = c(30:48, paste(35:46, "+", sep = ""), 1:9, paste(1:12, "+", sep = ""))
clothes = c("xs", "s", "m", "l", "xl", "xxl", "xxxl", "XS", "S", "M", "L", "XL", "XXL", "XXXL", 100:176)
unsized = c("unsized")
item$type = "rest"
...
# variables that are going to be used in prediction
item.final = item[, list(item_id = item_id, item_retrate = as.factor(item_retrate_c), item_avg.price = item_avg.price, 
...
# price_comp
joint$price_comp = NA
joint$price_comp = ((joint$item_price - joint$item_avg.price)/joint$item_avg.price)
...
joint$delivery_time = split(joint$delivery_time, joint$delitime, c, tag)
...
tag = c(1, 2, 3, 4, 5, 6)
joint$tau = split(joint$tau, joint$item_price, c, tag)
rm(c, tag, double)
